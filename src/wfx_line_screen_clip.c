...
/*   Updated: 2018/07/21 10:37:38 by wseegers         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static t_vec2i	get_clip(t_vec2i window, t_vec2 v1, t_vec2 v2, oct_code check)
{
	t_vec2i		temp;
...
int		clip(t_window *window, t_vec2i *p1, t_vec2i *p2)
{
	...
	while (1)
	{
		...
		temp = get_clip(VEC2I(window->width, window->height), VEC2_TO_F((*p1)), VEC2_TO_F((*p2)), check);
		if (check == p1_oct)
			*p1 = temp;
		else
			*p2 = temp;
