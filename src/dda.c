...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2018/07/24 14:10:38 by wseegers          #+#    #+#             */
/*   Updated: 2018/07/24 14:11:04 by wseegers         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void		get_hit_side(t_dda *dda, t_hit_report *hr, t_vec2 start,
					t_vec2 ray)
{
	double axis_dist;
...
t_hit_report	dda(t_vec2 start, t_vec2 direction, t_grid map)
{
	...
	dda_loop(&dda, map);
	if (!dda.yhit)
		hr.hit_dist = (dda.cell.x - start.x +
			(1 - dda.step.x) / 2) / direction.x;
	else
		hr.hit_dist = (dda.cell.y - start.y +
			(1 - dda.step.y) / 2) / direction.y;
